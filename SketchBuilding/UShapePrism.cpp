...
void UShapePrism::comp(const std::map<std::string, std::string>& name_map, std::vector<boost::shared_ptr<Shape> >& shapes) {
	...
	if (name_map.find("top") != name_map.end() && name_map.at("top") != "NIL") {
		...
		shapes.push_back(boost::shared_ptr<Shape>(new UShape(name_map.at("top"), _pivot, mat, _scope.x, _scope.y, _front_width, _back_depth, _color)));
	}

	// bottom face
	if (name_map.find("bottom") != name_map.end() && name_map.at("bottom") != "NIL" && _scope.z >= 0) {
		shapes.push_back(boost::shared_ptr<Shape>(new UShape(name_map.at("bottom"), _pivot, _modelMat, _scope.x, _scope.y, _front_width, _back_depth, _color)));
	}
	...
	if (name_map.find("front") != name_map.end() && name_map.at("front") != "NIL") {
		...
		shapes.push_back(boost::shared_ptr<Shape>(new Rectangle(name_map.at("front"), _pivot, glm::rotate(glm::rotate(glm::translate(_modelMat, glm::vec3(_front_width, 0, 0)), M_PI * 0.5f, glm::vec3(0, 0, 1)), rot_angle, glm::vec3(1, 0, 0)), _scope.y - _back_depth, fabs(_scope.z), _color)));
		shapes.push_back(boost::shared_ptr<Shape>(new Rectangle(name_map.at("front"), _pivot, glm::rotate(glm::translate(_modelMat, glm::vec3(_front_width, _scope.y - _back_depth, 0)), rot_angle, glm::vec3(1, 0, 0)), _scope.x - _front_width * 2, fabs(_scope.z), _color)));
		shapes.push_back(boost::shared_ptr<Shape>(new Rectangle(name_map.at("front"), _pivot, glm::rotate(glm::rotate(glm::translate(_modelMat, glm::vec3(_scope.x - _front_width, _scope.y - _back_depth, 0)), -M_PI * 0.5f, glm::vec3(0, 0, 1)), rot_angle, glm::vec3(1, 0, 0)), _scope.y - _back_depth, fabs(_scope.z), _color)));
	...
	if (name_map.find("side") != name_map.end() && name_map.at("side") != "NIL") {
		...
		if (name_map.find("front") == name_map.end()) {
			...
			shapes.push_back(boost::shared_ptr<Shape>(new Rectangle(name_map.at("side"), _pivot, glm::rotate(glm::rotate(glm::translate(_modelMat, glm::vec3(_front_width, 0, 0)), M_PI * 0.5f, glm::vec3(0, 0, 1)), rot_angle, glm::vec3(1, 0, 0)), _scope.y - _back_depth, fabs(_scope.z), _color)));
			shapes.push_back(boost::shared_ptr<Shape>(new Rectangle(name_map.at("side"), _pivot, glm::rotate(glm::translate(_modelMat, glm::vec3(_front_width, _scope.y - _back_depth, 0)), rot_angle, glm::vec3(1, 0, 0)), _scope.x - _front_width * 2, fabs(_scope.z), _color)));
			shapes.push_back(boost::shared_ptr<Shape>(new Rectangle(name_map.at("side"), _pivot, glm::rotate(glm::rotate(glm::translate(_modelMat, glm::vec3(_scope.x - _front_width, _scope.y - _back_depth, 0)), -M_PI * 0.5f, glm::vec3(0, 0, 1)), rot_angle, glm::vec3(1, 0, 0)), _scope.y - _back_depth, fabs(_scope.z), _color)));
...
void UShapePrism::offset(const std::string& name, float offsetDistance, const std::string& inside, const std::string& border, std::vector<boost::shared_ptr<Shape> >& shapes) {
	if (offsetDistance >= 0) {
		shapes.push_back(boost::shared_ptr<Shape>(new UShapePrism(inside, _pivot, glm::translate(_modelMat, glm::vec3(-offsetDistance, -offsetDistance, 0)), _scope.x + offsetDistance * 2, _scope.y + offsetDistance * 2, _scope.z, _front_width + offsetDistance * 2, _back_depth + offsetDistance * 2, _color)));
	}
	else {
		// not supported
	}
...
void UShapePrism::split(int splitAxis, const std::vector<float>& sizes, const std::vector<std::string>& names, std::vector<boost::shared_ptr<Shape> >& objects) {
	if (splitAxis == DIRECTION_X) {
		...
		for (int i = 0; i < sizes.size(); ++i) {
			if (names[i] != "NIL") {
				objects.push_back(boost::shared_ptr<Shape>(new UShapePrism(names[i], _pivot, mat, _scope.x, _scope.y, sizes[i], _front_width, _back_depth, _color)));
			}
