...
LShapePrism::LShapePrism(const std::string& name, const glm::mat4& pivot, const glm::mat4& modelMat, float width, float depth, float height, float front_width, float left_width, const glm::vec3& color) {
	this->_name = name;
	this->_removed = false;
	this->_pivot = pivot;
...
void LShapePrism::comp(const std::map<std::string, std::string>& name_map, std::vector<boost::shared_ptr<Shape> >& shapes) {
	...
	if (name_map.find("top") != name_map.end() && name_map.at("top") != "NIL") {
		...
		shapes.push_back(boost::shared_ptr<Shape>(new LShape(name_map.at("top"), _pivot, mat, _scope.x, _scope.y, _front_width, _left_width, _color)));
	}

	// bottom face
	if (name_map.find("bottom") != name_map.end() && name_map.at("bottom") != "NIL" && _scope.z >= 0) {
		shapes.push_back(boost::shared_ptr<Shape>(new LShape(name_map.at("bottom"), _pivot, _modelMat, _scope.x, _scope.y, _front_width, _left_width, _color)));
	}
...
void LShapePrism::offset(const std::string& name, float offsetDistance, const std::string& inside, const std::string& border, std::vector<boost::shared_ptr<Shape> >& shapes) {
	if (offsetDistance >= 0) {
		shapes.push_back(boost::shared_ptr<Shape>(new LShapePrism(inside, _pivot, glm::translate(_modelMat, glm::vec3(-offsetDistance, -offsetDistance, 0)), _scope.x + offsetDistance * 2, _scope.y + offsetDistance * 2, _scope.z, _front_width + offsetDistance * 2, _left_width + offsetDistance * 2, _color)));
	}
	else {
		// not supported
	}
...
void LShapePrism::split(int splitAxis, const std::vector<float>& sizes, const std::vector<std::string>& names, std::vector<boost::shared_ptr<Shape> >& objects) {
	if (splitAxis == DIRECTION_X) {
		...
		for (int i = 0; i < sizes.size(); ++i) {
			if (names[i] != "NIL") {
				objects.push_back(boost::shared_ptr<Shape>(new LShapePrism(names[i], _pivot, mat, _scope.x, _scope.y, sizes[i], _front_width, _left_width, _color)));
			}
...
void LShapePrism::generateGeometry(std::vector<glutils::Face>& faces, float opacity) const {
	...
	// top
	{
		...
		glutils::drawQuad(_front_width, _scope.y - _left_width, glm::vec4(_color, opacity), glm::translate(_pivot * _modelMat, glm::vec3(_scope.x - _front_width * 0.5f, (_scope.y - _left_width) * 0.5f, _scope.z)), vertices);
		glutils::drawQuad(_scope.x, _left_width, glm::vec4(_color, opacity), glm::translate(_pivot * _modelMat, glm::vec3(_scope.x * 0.5, _scope.y - _left_width * 0.5, _scope.z)), vertices);
	...
	if (_scope.z >= 0) {
		std::vector<Vertex> vertices;
		glutils::drawQuad(_front_width, _scope.y - _left_width, glm::vec4(_color, opacity), glm::rotate(glm::translate(_pivot * _modelMat, glm::vec3(_scope.x - _front_width * 0.5f, (_scope.y - _left_width) * 0.5f, 0)), M_PI, glm::vec3(1, 0, 0)), vertices);
		glutils::drawQuad(_scope.x, _left_width, glm::vec4(_color, opacity), glm::rotate(glm::translate(_pivot * _modelMat, glm::vec3(_scope.x * 0.5, _scope.y - _left_width * 0.5, 0)), M_PI, glm::vec3(1, 0, 0)), vertices);
		faces.push_back(glutils::Face(_name, vertices, _texture));
	}
		...
		// front
	{
		...
		glm::mat4 mat = _pivot * glm::rotate(glm::translate(_modelMat, glm::vec3((_scope.x - _front_width) * 0.5f, _scope.y - _left_width, _scope.z * 0.5f)), rot_angle, glm::vec3(1, 0, 0));
		...
		vertices.clear();
		mat = _pivot * glm::rotate(glm::rotate(glm::translate(_modelMat, glm::vec3(_scope.x - _front_width, (_scope.y - _left_width) * 0.5f, _scope.z * 0.5f)), M_PI * 0.5f, glm::vec3(0, 0, 1)), rot_angle, glm::vec3(1, 0, 0));
		glutils::drawQuad(_scope.y - _left_width, _scope.z, glm::vec4(_color, opacity), mat, vertices);
		faces.push_back(glutils::Face(_name, vertices));

		vertices.clear();
		mat = _pivot * glm::rotate(glm::translate(_modelMat, glm::vec3(_scope.x - _front_width * 0.5f, 0, _scope.z * 0.5)), rot_angle, glm::vec3(1, 0, 0));
		...
		// left
	{
		...
		glm::mat4 mat = _pivot * glm::rotate(glm::rotate(glm::translate(_modelMat, glm::vec3(0, _scope.y - _left_width * 0.5f, _scope.z * 0.5f)), -M_PI * 0.5f, glm::vec3(0, 0, 1)), rot_angle, glm::vec3(1, 0, 0));
		glutils::drawQuad(_left_width, _scope.z, glm::vec4(_color, opacity), mat, vertices);
		faces.push_back(glutils::Face(_name, vertices));
	}
}
